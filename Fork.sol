 ...
 */
contract Context {
    // Empty internal constructor, to prevent people from forkakenly deploying
...
contract Fork is ERC20, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 FORK-WFTM LP -> FORK pool
    uint256 public constant INITIAL_FORK_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the FORK ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("FORK", "FORK") {
        // Mints 1 FORK to contract creator for initial pool setup
    ...
    function _getForkPrice() internal view returns (uint256 _forkPrice) {
        try IOracle(forkOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Fork: failed to fetch FORK price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _forkPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_forkPrice >= taxTiersTwaps[tierId]) {
    ...
    function setForkOracle(address _forkOracle) public onlyOperatorOrTaxOffice {
        require(_forkOracle != address(0), "oracle address cannot be 0 address");
        forkOracle = _forkOracle;
    }
    ...
    /**
     * @notice Operator mints FORK to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of FORK to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentForkPrice = _getForkPrice();
            currentTaxRate = _updateTaxRate(currentForkPrice);
            if (currentForkPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_forkPool != address(0), "!_forkPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_forkPool, INITIAL_FORK_POOL_DISTRIBUTION);
